...
description: 診斷並修復 Expo / EAS build 失敗（雲端或 `eas build --local`），以及 CNG 專案的原生建置問題。當 agent 看到 `npm ci` 拒絕 lockfile、EAS 壓縮包異常肥大、expo-updates runtime/指紋不合、雲端建置額度用完、本地 iOS Swift 編譯錯誤、Android OutOfMemoryError Metaspace、gradle GlobalCacheLocations 錯誤、訊息被吞的「node exited 1」、Android release lint ExtraTranslation 時使用。內含九條來自 2026-08 便便植物園上架的真實病歷（症狀→根因→修法），以及 EAS log 解壓、保留工作目錄、手動重放 gradle 的診斷工具箱。
---

# eas-build-doctor

EAS build 失敗病歷本。每條都是 2026-08「便便植物園」（gut-game，Expo SDK 54 / expo-modules 57）
...
- **修法**：整顆重生。
  ...
  ```
  同一專案此症狀出現兩次（gut-game `4a688f5`、`db32bb9`、`71ce9bf`），兩次都是重生法治好。
...
- **修法**：`.easignore` 必須重列 `.gitignore` 原本排除的一切（gut-game `badf733`）：
  ```gitignore
  # 專案自己的大型素材
  tmp/
  docs/
...
- **修法**：
  ...
  2. 所有對 node_modules 的手改一律 `npx patch-package <pkg>` 進 `patches/`，
     `"postinstall": "patch-package"`，兩端安裝後 byte 一致（gut-game `495883b`）。
...
- **修法**：最小修正（改用 `.magnitude`，或明確 qualify）→ `npx patch-package expo-modules-jsi`
  → patch 進 repo（gut-game `495883b`）。**追上游 issue，上游修了就刪 patch。**
...
- **修法**：在 `eas-build-post-install` hook 裡 sed（gut-game `5beb9d2`）：
  ```json
...
- **根因**：hook 用 `echo >>` 續寫 `gradle.properties`，而模板產出的該檔**結尾沒有換行**。
  ...
  expo-autolinking 的 `--watched-directories-serialized` 參數，node 直接炸（gut-game `0d56927`）。
...
- **修法**：locales 檔平台分開（gut-game `aeba9e0`）：
  ```json
  {
    "ios":     { "CFBundleDisplayName": "便便植物園" },
    "android": { "app_name": "便便植物園" }
  }
  ```
