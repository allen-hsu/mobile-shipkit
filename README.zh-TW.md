...
萃取自一次真實的雙商店上架(2026-08):從零商店資料到 Apple + Google 送審,
途中修掉十餘個真實故障。本 repo 把那些經驗固化成可重用的形式。
...
| skill | 一句話 |
|---|---|
| `eas-build-doctor` | EAS build 失敗病歷本:每次真實失敗的根因與修法 |
| `submit-apple` | 疊在 asc skills 之上的 App Store 踩點 |
| `eas-ota-discipline` | OTA 指紋鐵律:更新為什麼沒人收到 |
| `submit-google` | Play 上架全流程:API 能做的全做、Console-only 清單明列 |
| `app-scout` | 要做什麼：用公開商店資料掃 geo-gap / zombies / weak / paid-gap |
| `store-screenshots` | 五步驟截圖流程：brief → screens → look → sizes → review → render |
| `store-art` | HTML/CSS + Playwright 渲染器：30 種可組合風格 × 30 種版型、一份 JSON manifest、元素層（UI 碎片／戳章／引言）、Codex 生吉祥物與貼紙、風格型錄、審稿頁、商店規範護欄、feature graphic、icon 組 |
